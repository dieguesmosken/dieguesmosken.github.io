	...
	if((!empty($email)) AND (!empty($senha))){
		...
		if($resultado_usuario){
			...
			if($senha == $row_usuario['pass_usr'] & $row_usuario['tipo_usr'] == 1 ){
				...
				$_SESSION['pass'] = $row_usuario['pass_usr'];
				$_SESSION['tipousr'] = $row_usuario['tipo_usr'];
				$_SESSION['dtnasc'] = $row_usuario['dtnasc_usr'];
				$_SESSION['cpf_usr'] = $row_usuario['cpf_usr'];
				$_SESSION['dtmk'] = $row_usuario['dtcriado_usr'];
				$_SESSION['dtmd'] = $row_usuario['dtmodificado_usr'];
				...
				$_SESSION['msg'] = "Não encontrado/Senha incorreta!";
				header("Location: login.php");
			
			}
		}
	}else{
		$_SESSION['msg'] = "Usuario não encontrado Cadastre - se!";
